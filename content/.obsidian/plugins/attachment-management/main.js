...
var ja = {
  ...
  settings: {
    ...
    excludedAttachmentPaths: {
      name: "\u9664\u5916\u3055\u308C\u308B\u6DFB\u4ED8\u30D5\u30A1\u30A4\u30EB\u306E\u30D1\u30B9",
      desc: "\u30D5\u30A9\u30EB\u30C0\u540D\u306E\u30D5\u30EB\u30D1\u30B9\uFF08\u5927\u6587\u5B57\u3068\u5C0F\u6587\u5B57\u3092\u533A\u5225\u3057\u3001\u5148\u982D\u306E\u30B9\u30E9\u30C3\u30B7\u30E5\u300C/\u300D\u306F\u542B\u3081\u306A\u3044\uFF09\u3092\u30BB\u30DF\u30B3\u30ED\u30F3\uFF08;\uFF09\u3067\u533A\u5207\u3063\u3066\u6307\u5B9A\u3057\u3066\u304F\u3060\u3055\u3044\u3002\u3053\u306E\u30D5\u30A9\u30EB\u30C0\u306B\u30EA\u30F3\u30AF\u3055\u308C\u3066\u3044\u308B\u6DFB\u4ED8\u30D5\u30A1\u30A4\u30EB\u306F\u3001\u540D\u524D\u5909\u66F4\u304A\u3088\u3073\u79FB\u52D5\u306E\u5BFE\u8C61\u304B\u3089\u9664\u5916\u3055\u308C\u307E\u3059\u3002"
    },
    excludeSubpaths: {
      name: "\u30B5\u30D6\u30D1\u30B9\u3092\u9664\u5916",
      desc: "\u3053\u306E\u30AA\u30D7\u30B7\u30E7\u30F3\u3092\u30AA\u30F3\u306B\u3059\u308B\u3068\u3001\u4E0A\u8A18\u3067\u6307\u5B9A\u3057\u305F\u30D5\u30A9\u30EB\u30C0\u30D1\u30B9\u306E\u3059\u3079\u3066\u306E\u30B5\u30D6\u30D5\u30A9\u30EB\u30C0\u3082\u9664\u5916\u3055\u308C\u307E\u3059\u3002"
    },
    excludeAttachmentSubpaths: {
      name: "Exclude attachment subpaths",
...
var en = {
  ...
  settings: {
    ...
    excludedAttachmentPaths: {
      name: "Excluded attachment paths",
      desc: "Provide the full path of the folder names (case sensitive and without leading slash '/') divided by semicolon (;). Any attachments linked attachments to this folder will be excluded from renaming and moving"
    },
    excludeSubpaths: {
...
var zhCn = {
  ...
  settings: {
    ...
    excludedAttachmentPaths: {
      name: "\u88AB\u6392\u9664\u7684\u9644\u4EF6\u8DEF\u5F84",
      desc: "\u8BF7\u63D0\u4F9B\u6587\u4EF6\u5939\u540D\u79F0\u7684\u5B8C\u6574\u8DEF\u5F84\uFF08\u533A\u5206\u5927\u5C0F\u5199\uFF0C\u4E14\u4E0D\u5305\u542B\u5F00\u5934\u7684\u659C\u6760\u201C/\u201D\uFF09\uFF0C\u5E76\u7528\u5206\u53F7\uFF08;\uFF09\u5206\u9694\u3002\u4E0E\u8BE5\u6587\u4EF6\u5939\u5173\u8054\u7684\u4EFB\u4F55\u9644\u4EF6\u90FD\u5C06\u88AB\u6392\u9664\u5728\u91CD\u547D\u540D\u548C\u79FB\u52A8\u64CD\u4F5C\u4E4B\u5916\u3002"
    },
    excludeSubpaths: {
      name: "\u6392\u9664\u5B50\u8DEF\u5F84",
...
var DEFAULT_SETTINGS = {
  ...
  excludedAttachmentPaths: "",
  excludedAttachmentPathsArray: [],
  excludeAttachmentSubpaths: true
};
var AttachmentManagementSettingTab = class extends import_obsidian4.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian4.Setting(containerEl).setName(t("settings.excludedAttachmentPaths.name")).setDesc(t("settings.excludedAttachmentPaths.desc")).addTextArea((component) => {
      component.setValue(this.plugin.settings.excludedAttachmentPaths).onChange(async (value) => {
        this.plugin.settings.excludedAttachmentPaths = value;
        const { splittedPaths } = this.splitPath(value);
        this.plugin.settings.excludedAttachmentPathsArray = splittedPaths;
        debugLog("setting - excluded attachment paths:" + value, splittedPaths);
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian4.Setting(containerEl).setName(t("settings.excludeAttachmentSubpaths.name")).setDesc(t("settings.excludeAttachmentSubpaths.desc")).addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.excludeAttachmentSubpaths).onChange(async (value) => {
        debugLog("setting - excluded attachment subpaths:" + value);
        this.plugin.settings.excludeAttachmentSubpaths = value;
        await this.plugin.saveSettings();
      })
    );
...
// src/attachmentExclude.ts
function isAttachmentExcluded(path2, settings) {
  debugLog("excludeAttachmentPathsArray: ", settings.excludedAttachmentPathsArray);
  for (const excludedPath of settings.excludedAttachmentPathsArray) {
    if (excludedPath.length === 0) {
      continue;
    }
    if (settings.excludeAttachmentSubpaths && path2.startsWith(excludedPath)) {
      debugLog("isAttachmentExcluded: ", path2);
      return true;
    } else {
      if (path2 === excludedPath) {
        return true;
  ...
  async rearrangeAttachment(type, file, oldPath) {
    ...
    for (const obNote of Object.keys(attachments)) {
      ...
      for (let link of attachments[obNote]) {
        ...
        if (linkFile.parent && isAttachmentExcluded(linkFile.parent.path, this.pluginSettings)) {
          debugLog("create - excluded attachment file by folder", linkFile);
          continue;
        }
...
var CreateHandler = class {
  ...
  processAttach(attach, source) {
    ...
    if (attach.parent && isAttachmentExcluded(attach.parent.path, this.settings)) {
      debugLog("processAttach - not a file or exclude attachment path:", attach.path);
      return;
    }
    ...
    this.app.workspace.onLayoutReady(() => {
      ...
      this.registerEvent(
        this.app.vault.on("create", async (file) => {
          ...
          if (isAttachment(this.app, this.settings, file) && file.parent && isAttachmentExcluded(file.parent.path, this.settings)) {
            debugLog("create - excluded attachment file by folder", file);
            return;
          }
              ...
              this.app.vault.adapter.exists(f.path, true).then((exist) => {
                if (exist) {
                  ...
                  if (file.extension == "md" && data.indexOf(link) != -1 || file.extension == "canvas" && data.indexOf(f.path) != -1) {
                    if (isAttachment(this.app, this.settings, f) && f.parent && isAttachmentExcluded(f.parent.path, this.settings)) {
                      debugLog("create - excluded attachment file by extension", file);
                      return;
                    }
